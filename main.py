...
# ---------------------------------------------------------------------------
# 4. Playwright를 이용한 네이버 플레이스 정밀 크롤링 (아코디언 자동 펼침 포함)
# ---------------------------------------------------------------------------
def crawl_naver_place_with_playwright(query, page):
    ...
    try:
        ...
        if place_id:
            ...
            # 💡 [핵심] 아코디언(펼쳐보기, 더보기 등) 자동 클릭하여 숨겨진 정보 노출
            try:
                # '펼쳐보기', '더보기' 버튼이나 관련 클래스/텍스트를 찾아 클릭 시도
...
def parse_flags(text, raw_html, name=""):
    ...
    # 아코디언이 펼쳐진 텍스트 구조(예: 월 09:00 - 19:00 등)를 포착하는 정규식
...
# ---------------------------------------------------------------------------
# 6. 메인 파이프라인 실행 (Playwright 브라우저 제어 적용)
# ---------------------------------------------------------------------------
def main():
    LIMIT = 90
    ...
    print(f"🚀 총 {len(hospitals)}개의 타겟 병원을 찾았습니다. Playwright 아코디언 자동 펼침 크롤링을 시작합니다!\n")
    print("=" * 70)

    with sync_playwright() as p:
        # 헤드리스 크롬 브라우저 띄우기 (모바일 환경 에뮬레이션)
    ...
    print("\n✨ 생명마루한의원 안산점 플레이스 상위 노출 분석을 위한 아코디언 확장 정밀 크롤링이 끝났습니다.")
