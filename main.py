...
# ---------------------------------------------------------------------------
# 5. 한방 / 양방 / 한양방(협진) 정밀 판별 함수 (단순 진료과목 오진 100% 차단)
# ---------------------------------------------------------------------------
def determine_hanbang_type(name, raw_text):
    n = name.lower()
    t = raw_text.lower() if raw_text else ""
    
    # 🎯 1. 상호명에 '한의원' 또는 '한방병원'이 명시된 경우 -> 100% 한방
    if "한의원" in n or "한방병원" in n:
        return "한방"
    
    # 🎯 2. '협진'을 직접적으로 의미하는 엄격한 키워드만 한양방으로 인정
    # (단순 개별 진료과목명인 '한방소아과', '침구과', '한방내과' 등은 오진 방지를 위해 제외)
